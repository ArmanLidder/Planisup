...
export class UserService {
  ...
  async getEmployees(userRoles: UserRole[]): Promise<User[]> {
    try {
      const users = await UserModel.find({ role: { $in: userRoles } })
        .select("-__v")
        .lean<IUser[]>();
      return users.map((user) => convertUserInterface(user));
    } catch (error) {
      this.logger.error(
        `Error fetching all ${userRoles.join(", ")} users: ${error}`
      );
  ...
  async updateUserRole(
    ...
    try {
      const query = departement
        ? { role: newRole, department: departement }
        : { role: newRole, department: "" };
      const updatedUser = await UserModel.findByIdAndUpdate(userId, query, {
        new: true,
      });
      ...
      this.logger.info(
        `User ${updatedUser.usercode} role updated to ${newRole}`
      );
  ...
  async getUserByUsercode(usercode: string): Promise<User | null> {
    try {
      ...
      this.logger.error(
        `Error fetching user by usercode ${usercode}: ${error}`
      );
      throw new Error("Failed to fetch user");
    }
  }
  ...
  async createStudent(student: Partial<User>): Promise<User | null> {
    try {
      ...
      this.logger.error(
        `Error creating student user ${student.usercode}: ${error}`
      );
  ...
  async getStudentsWithUnsubmittedStudyPlan(): Promise<User[]> {
    try {
      ...
      this.logger.error(
        `Error fetching students with unsubmitted study plans: ${error}`
      );
